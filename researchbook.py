...
class ResearchBook:
    def __init__(self):
        ...
        self.db1_driver = GraphDatabase.driver(
            os.getenv("NEO4J_URI_DB1", "neo4j+s://localhost:7688"),
            auth=(os.getenv("NEO4J_USER", "neo4j"), os.getenv("NEO4J_PASSWORD"))
        )

        # Database 2 - Thesis Relationships
        self.db2_driver = GraphDatabase.driver(
            os.getenv("NEO4J_URI_DB2", "neo4j+s://localhost:7687"),
            auth=(os.getenv("NEO4J_USER", "neo4j"), os.getenv("NEO4J_PASSWORD"))
        )

        # LightLLM API
        self.llm_url = os.getenv("LIGHTLLM_URL", "https://localhost:4000/v1/chat/completions")
    ...
    def _search_experts_db1(self, topic: str, limit: int) -> List[Dict]:
        ...
        with self.db1_driver.session(database="neo4j") as session:
            ...
            WHERE toLower(pub.Keywords) CONTAINS toLower($topic) OR
                  ...
                  toLower(pub.name) CONTAINS toLower($topic)
            WITH p, count(pub) as relevant_pubs, collect(pub.name)[..3] as sample_pubs
...
# API integrations (REST + auth + timeouts)
# Where: ai_query() (requests.post, headers, JSON, timeout, error paths). Why: Same pattern for Slack/Figma/GitLab webhooks & polling.

# Prompt engineering for structured outputs
# Where: _create_person_analysis_prompt(), _create_expert_ranking_prompt(). Why: Task-scoped prompts for summaries/reports in Slack.

# LLM orchestration & token budgeting
# Where: ai_query(..., max_tokens, temperature=0.3). Why: Control latency/cost; ensure deterministic, parseable replies.

# Graph DB querying (Neo4j/Cypher)
# Where: _get_researcher_profile_db1(), _get_thesis_activities_db2(), _search_experts_db1/db2(). Why: Mirrors querying component graphs (design system vs code).

# Entity aggregation across sources
# Where: lookup_person() (merge DB1/DB2), find_expert() (combine + dedupe). Why: Join Figma+GitLab facts into one compliance view.

# Ranking & scoring pipeline
# Where: _merge_expert_results() (combined_score, weighting). Why: Prioritize mismatches or “most risky components” in reports.

# Pagination/limits for performance
# Where: LIMIT in all Cypher queries; sample lists ([..3]). Why: Prevents memory spikes; keeps Slack responses snappy.

# Resilient data shaping
# Where: Safe dict assembly (defaults, or []), filtered affiliations. Why: Robust to partial API data (missing fields, nulls).

# Clear separation of concerns
# Where: LLM in ai_query, DB in _get_*, orchestration in public methods. Why: Easy to swap in Slack/Figma/GitLab connectors.

# Report-ready narratives
# Where: AI summaries in lookup_person() and find_expert(). Why: Turn raw checks into stakeholder-friendly Slack posts.

# Topic/keyword search across text fields
# Where: _search_experts_db1/db2() (title/abstract/keywords). Why: Natural-language queries about components/properties in the chatbot.

# Operational hygiene (timeouts/error text)
# Where: ai_query() returns explicit “AI Error: …”. Why: Graceful degradation when a service is down; notify in Slack.